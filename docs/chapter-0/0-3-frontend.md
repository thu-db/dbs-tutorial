...
现在我们在这方面进行给出了一定的规范，以便完成手动检查和自动化测试。我们将手动检查时的模式称作“交互模式”，将面向自动化测试的模式称作“批处理模式”。
...
注意，我们默认使用数据导入时数据是“安全”的，即格式正确、满足完整性约定等，无需在这部分做对应检查。
...
在自动化测试时对表之后的内容请参考上述示例使用空格，连续的多个空格会被视为一个空格，但是缺少空格或不该有的空格可能会被判定为错误。

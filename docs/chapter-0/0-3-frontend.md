...
- 仅下述语句需要给出对应表头并输出内容，其他直接输出分隔符即可：
    - `SELECT` 语句，注意表头无需精简，在 `SELECT` 后的 `selector` 不是 `*` 时，表头与 `selector` 一致即可；在 `selector` 是 `*` 时，如果是单表则表头为列名，如果是多表则表头为 `<table>.<column>`，如 `student.id`，我们保证自动化测试时不会在 `selector` 中留坑
    - `SHOW` 开头的语句，对于 `SHOW XX` 的语句，输出的表头即为 `XX`
    - `INSERT`、`DELETE`、`UPDATE` 语句均输出一个表头为 `rows`、内容为一个整数的表来展示增/删/改的行数。
...
- 需要输出多行数据时，如果不涉及 `ORDER BY` 则行序可以是任意的，否则应该按照 `ORDER BY` 的要求给出，排序关键字相同的若干行顺序是任意的。
- 浮点数统一精确到小数点后两位；
...
注意，我们默认使用数据导入时数据是“安全”的，即格式正确、满足完整性约定等，无需在这部分做对应检查，你应当采用尽可能高效的方法将数据批量导入，例如填满一页记录后一次写入理应比逐条写入记录更快。
...
注意，在自动化测试时**输出完表格后，在输出约束信息前必须输出一个空行**作为分隔（如果没有任何约束信息，则空行输出与否不影响结果正确性），并且下面的格式需要严格按照上述标准。由于我们的文法允许匿名主外键、索引，而有的同学会为它们设定默认名字，因此对于本来没有名字的约束即便你输出了自定义名字也不会影响正确性，但是本来有名字的约束你需要准确输出名字。
...
对于建立外键是否显示索引、建立主键是否自动创建 UNIQUE 约束、删除主外键时是否删除手动创建过的索引等问题，文档没有做细节要求，取决于自己的实现标准。为了规范自动化测试的要求，这里将因为创建外键、逐渐等约束带来的索引称作“隐式索引”，通过包含 `INDEX` 关键字的语句手动创建的索引称作“显式索引”，我们统一要求 `DESC` 只显示显式索引，不显示隐式索引。同时我们保证在自动化测试的测例中不会出现隐式索引和显式索引混淆的情况，例如保证如果已经建立联合索引 $(C_1,C_2)$，则在删除它之前一定不会再建立主键 $(C_1,C_2)$，反之亦然。

...
- 仅下述语句需要给出对应表头并输出内容（即便没有内容也应该输出一个表头），其他直接输出分隔符即可：
    - `SELECT` 语句，注意表头不必精简，在 `SELECT` 后的 `selector` 不是 `*` 时，表头与 `selector` 一致即可；在 `selector` 是 `*` 时，如果是单表则表头为列名，如果是多表则表头为 `<table>.<column>`，如 `student.id`。注意如果你愿意也可以改为“没有歧义的字段省略表名”，评测器对于可以精简的表头只会检查列名（此时即便表名错误也不会判定为错误，这不算评测器 BUG）

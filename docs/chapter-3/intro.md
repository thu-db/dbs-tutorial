...
**对于外键约束的列和被外键引用的列，本实验不做任何限定。**我们同样介绍一些实际数据库的情况。在MySQL中，如果外键约束作用于某列或某几列上，且这些列不存在合适的索引，自动在这些列上创建索引；在被外键引用的列上必须有索引。在PostgreSQL中，外键约束不会自动在这些列上创建索引；但是被其引用的列必须是主键或受到唯一约束，这样的列在PostgreSQL中会被自动创建索引。
